...
$(function(){
	...
	<div class="page-container2">
        ...
        <div class="row-fluid" <c:if test="${empty wbList}">style="display: none" </c:if>>
			...
			<div class="row-fluid"
				<c:if test="${empty mgList}">style="display: none" </c:if>>
				<div class="con">
					<%-- <div class="title">
						<div class="titleBlue">${mgTitle}</div>
					</div> --%>
					<div class="hwBox">
						<div style="word-break: break-all; width: 100%; line-height: 200%;">
							${reportTime},针对“${reportNameAll}”监测方案进行统计,网络上该事件共新增信息${totalNum}条,建议重点关注以下信息:
						</div>
					</div>
					<div class="main_list">
						<c:if test="${empty mgList}">
						...
						<c:if test="${not empty mgList}">
							<ul>
								<c:forEach var="mg" items="${mgList}" begin="0" end="9" varStatus="status">
									<li style="padding: 10px 0px 0px 10px; min-height: initial">
