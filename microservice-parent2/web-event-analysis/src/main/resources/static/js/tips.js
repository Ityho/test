...
function operateNews(opt_type,checkedIds, importType, account,timeDomain,starttime,endtime){
//$('.submitBtn').unbind("click"); //移除click
	...
	if(clickFlag==0){
		clickFlag=1;
		NewsOperate.operateNewsSenior(opt_type,checkedIds, importType, account,timeDomain,starttime,endtime,
				CallBackSenior);
	}
...
function importNewsSenior(importNewType, folderId, account) {
    ...
    if(importNewType==1&&(parseInt(collectNum)+checkNum)>2000){
        $("#cancelBtn").html("确认");
        $("#submitBtn").html("去收藏夹");
        showMsgInfo(0,"失败啦，收藏夹只提供2000条信息加入哦！",1);
        $("#submitBtn").click(function(){
            location.href = "/collect.shtml";
        });
        return;
    }
	...
	/**if (checkedMenu == "0") {
		...
		operateNews("insert",checkedIds, importType, account,timeDomain,starttime,endtime);
