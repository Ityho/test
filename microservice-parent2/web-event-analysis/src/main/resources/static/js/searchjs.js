...
$(function(){
	...
	$("#searchKeyword").on('keypress',function(e) {
		var searchKeyword_value=$("#searchKeyword").val();
	...
	$("#goSearch").on("click",function(){
		...
		if(i>=2){
			if(!$("#adminId").val() && !$("#platform").val()){
				location.href = actionBase+"/indexLocal";
				return false;
			}
...
//	$("#goHomeSearch").on('keypress',function(e) {
//        var keycode = e.keyCode;
//        var searchName = $(this).val();
//        if(keycode=='13') {
//             e.preventDefault();
...
function goHomeSearchTwo(){
	...
	if(!checkKeywordFilter($searchKeyword.val())){
		setTimeout(function(){
			swal("关键词只能包含中文、英文和数字");
		},1);
		return false;
	}
	var searchKeyword=$('#searchKeyword').attr("placeholder");
	var searchKeyword_value=encodeURIComponent($('#searchKeyword').val());

//		 if(searchKeyword=="搜索人名"){
//			 window.location.href=njxBasePath+"/view/hotSearch/goHotSearch.shtml";
//		 }else{
...
//				 if(searchKeyword=="搜索人名"){//微博情绪
//					 $("form[name='searchForm']").attr("action",njxBasePath+"/view/search/goSearch.shtml?searchShareCode="+result.obj+"#9");
//					 $("form[name='searchForm']").submit();
////					 $("form[name='searchForm']").attr("action",njxBasePath+"/view/moodMap/goWeiboMood.shtml?searchShareCode="+result.obj+"#1");
////					 $("form[name='searchForm']").submit();
//				 }else{
//
////					 $("form[name='searchForm']").attr("action",njxBasePath+"/view/hotSearch/goHotSearch.shtml?searchShareCode="+result.obj);
////					 $("form[name='searchForm']").submit();
//				 }
			}
		}});
...
function goToHotSearch(index){

	$.ajax({url:njxBasePath+"/view/search/doSearchShareCode.shtml",cache:false,async:false,success:function(result){
			if(result.status){
				$("#fmGoHot"+index).attr("action",njxBasePath+"/view/search/goSearch.shtml?searchShareCode="+result.obj+"#1");
				$("#fmGoHot"+index).submit();
			}
		}});
}
