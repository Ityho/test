...
$(function(){
	...
	$("#goSearch").on("click",function(){
		...
		var i = 0,keywordstr1 = "",keywordstr2 = "",keywordstr3 = "",keywordsJson={};
		...
		if(i>=2 && bs.versions.android && $("#platform").val() == "android"){
			try {
				if($("#isHotRelatedApp").val() == 1){
					window.AndroidModel.compare(JSON.stringify(keywordsJson));
				}else{
					window.AndroidModel.compare(keywordstr1, keywordstr2,keywordstr3);
				}
			} catch (e) {}
			return false;
		}
		if(i>=2 && bs.versions.ios && $("#platform").val() == "iOS"){
			if($("#isHotRelatedApp").val() == 1){
				window.webkit.messageHandlers.CompareModel.postMessage({body:[keywordsJson]});
			}else{
				window.webkit.messageHandlers.CompareModel.postMessage({body:[keywordstr1, keywordstr2, keywordstr3]});
			}
			return false;
		}
		if(i>=2){
...
function goSearch(that,time){
	$(that).css({"background":"#fc8425","color":"#ffffff"});
	$(that).siblings().css({"background":"#f7f7f7","color":"#fd8c25"});
		...
		$.ajax({
			...
			success:function(data){
				if(data.status){
					...
					divHtml = divHtml+"</ul><div class='emCancel' style = 'background:#f2f2f2;line-height:35px;'>关闭</div></section>";
