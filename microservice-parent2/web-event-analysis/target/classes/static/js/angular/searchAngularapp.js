	...
	bigpipe.ready('doEventTrends',function(result){
		...
		if(result.status){
			...
			for(var key in result.obj){
				...
				if(!result.obj[key].statList.length){
					continue;
				}
				var topValue = result.obj[key].statList[0].total.toFixed(2),topTime,subMsg;
				var topTime = result.obj[key].statList[0].name;
				for(var i = 0;i<result.obj[key].statList.length;i++){
					opt.series[g].showAllSymbol = false;
					opt.series[g].data[i] = result.obj[key].statList[i].total.toFixed(2);
					opt.xAxis[0].data[i] = result.obj[key].statList[i].name+":00";

					if(parseFloat(result.obj[key].statList[i].total) > parseFloat(topValue)){
						topValue = result.obj[key].statList[i].total.toFixed(2);
						topTime = result.obj[key].statList[i].name;
					}
				}
				if(key==0){
	...
	bigpipe.ready('doSourceStatistics',function(result){
		if(result.status){
			...
			for(var key in result.obj){
				...
				for(var i = result.obj[key].length-1,j=0;i>=0;i--,j++){
					opt.series[g].data[j] = {value:result.obj[key][i].num,name:result.obj[key][i].name};

					opt.yAxis[0].data[j] = result.obj[key][i].name;

					if(i<3 && result.obj[key].length>3){
						topValue += (result.obj[key][2-i].name+"("+result.obj[key][2-i].percentStr) +")、";
						topName += result.obj[key][2-i].name + "、";
					}else if(i<3 && result.obj[key].length<3){
						topValue += (result.obj[key][result.obj[key].length-i].name+"("+result.obj[key][result.obj[key].length-i].percentStr) +")、";
						topName += result.obj[key][result.obj[key].length-i].name + "、";
					}
		...
		if(result.status){
			...
			for(var key in result.obj){
				...
				if(result.obj[key][0]){
					...
					for(j = 0;j<result.obj[key].length;j++){
						console.log("colors:"+g+":"+colors[g]);
						opt.series[0].data[i] = {name:result.obj[key][j].name,itemStyle:{normal:{color:colors[g%3]}}};//设置每一个节点的数据
						opt.series[0].links[i] = {source:result.obj[key][0].name,target:result.obj[key][j].name};
						if(j==0){
							opt.series[0].data[i] = $.extend(true,opt.series[0].data[i],{name:result.obj[key][j].name,symbolSize:30});
						}
						if(j!=0&&j<4){
							topName += result.obj[key][j].name+"("+ result.obj[key][j].percentStr + ")、";
						}
						i++;
					}
					topName = topName.substring(0,topName.length-1);
