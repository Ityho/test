...
<link href="css/fxmail.css" rel="stylesheet" type="text/css" />
</head>
     ...
     <div class="info">
       <div class="info_tit">
       	<a href="${con.webpageUrl}">
       		...
       		<s:if test='#attr.con.captureWebsiteName=="新浪微博"'>
				<img src='images/sinawb.png' width="25">
			</s:if>
			<s:if test='#attr.con.captureWebsiteName=="微信"'>
				<img src='images/wx.png' width="25">
			</s:if>
			<s:if test='#attr.con.captureWebsiteName=="腾讯微博"'>
				<img src='images/qqwb.png' width="25">
			</s:if>
       		<s:if test='#attr.con.originType!="wb"&&#attr.con.author!=null&&#attr.con.author!=""'>
       ...
       <div class="attr">
         <ul>
           <li>采集自：
           	<s:if test='#attr.con.captureWebsiteName!=null'><s:property value="#attr.con.captureWebsiteName" /></s:if>
			<s:else>未知</s:else>
           </li>
           <li>作者： ${con.author}</li>
           <li>发布时间：
           		<s:set name="pdate"	value="#attr.con.published" />
	       ...
	       <!-- 微博content的展示 start -->
					<div class="conBox">
						...
						<s:if test="#attr.con.images!=null">
							<p class="photo photo2 thumbnails" id="gallery_<s:property value="#con.id" />">
