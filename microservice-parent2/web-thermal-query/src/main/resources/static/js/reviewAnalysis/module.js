...
$(function() {
    ...
    $("[data-toggle='popover']").popover();//全局弹出提示效果


//以下是panel面板标题栏右侧工具js效果
//panel full全屏缩放
     var full = 1;
    $("a.full").on("click", function() {
        if(full) {
            full--;
            $(this).parents(".panel").addClass("fullCon");
            $("body").addClass("o_hide");
            $(this).html("<i class='icon-nofull'></i>").attr("data-original-title","缩小");
        } else {
            full++;
            $(this).parents(".panel").removeClass("fullCon");
            $("body").removeClass("o_hide");
            $(this).html("<i class='icon-full'></i>").attr("data-original-title","全屏");
        }
    });
//panel collapse隐藏/展开内容
    $('.panel-body').on('hide.bs.collapse', function () {
        $(this).parents(".panel").find(".tools a.collapse").find("i").addClass("icon-angle-up").removeClass("icon-angle-down").attr("data-original-title","展开");
    });
    $('.panel-body').on('show.bs.collapse', function () {
        $(this).parents(".panel").find(".tools a.collapse").find("i").addClass("icon-angle-down").removeClass("icon-angle-up").attr("data-original-title","收起");
    });
...
//var collapse = 1;
//$(" .panel .panel-heading").find(".tools a.collapse").on("click", function() {
//	var $this = $(this);
//	if(collapse) {
//		collapse--;
//		$this.parents(".panel").find(".panel-body").css("display","none");
//  	$this.parents(".panel").find(".panel-footer").css("display","none");
//		$this.html("<i class='icon-angle-up'></i>").attr("data-original-title","展开");
//	} else {
//		collapse++;
//		$this.parents(".panel").find(".panel-body").css("display","block");
//  	$this.parents(".panel").find(".panel-footer").css("display","block");
//		$this.html("<i class='icon-angle-down'></i>").attr("data-original-title","收起");
//	}
//});

//panel 刷新内容
    /*var loaders = '<i class="icon-reload-a fa-spin"></i>';*/
    /*$(" .panel .panel-heading").find(".tools a.reload").on("click", function() {
        $(this).parents(".panel").find(".panel-body").loader('show',loaders);
        setTimeout(function(){
