			...
			init : function(params) {
				...
				if (elem.next('div').hasClass(elemClass)) {
					...
					if (_options.showCloseBut) {
						closeBut = $("<button type='button' class='close'><i class='icon-close'></i></button>");
					}
...
//			eObjMinWidth = parseInt(elemObj.css('minWidth')),
			eObjMaxWidth = parseInt(elemObj.css('Width')),
			eObjMinWidth = parseInt(elemObj.css('Width')),
			eObjWidth,
		...
		var methods = {
			init : function(params) {
				...
				elemObj.mouseenter(function() {
					...
					elemCur.removeClass(animClassOld).addClass(animClassNew).fadeIn("200");
				});
