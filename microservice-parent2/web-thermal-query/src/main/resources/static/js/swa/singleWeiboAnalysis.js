$(function(){
	...
	var fire1 = "";var fire2="";var time1 = "";
});
// 分析微博
function goAnalysisWeibo(weiboId, weiboUrl, retry) {
	if ((weiboUrl != null && weiboUrl != '') || (weiboId != null && weiboId != '')) {
		var url = actionBase + '/i/singleWeiboAnalysis/analysis.shtml';
		var params = {};
...
function myWeibo(uid) {
	if (uid != null && uid != "") {
		$.ajax({
			url : actionBase + '/i/singleWeiboAnalysis/myWeibo.action',
			type : 'POST',
			data : {
				'uid':uid
			},
			success : function(result) {
				if (!$.isEmptyObject(result)) {
					...
					$.each(result.statusList, function(i, n) {
						var html = '<li>';
						html += '		<div class="mwbcontext img_small">';
						...
						if (n.retweetedStatus != null) {
							...
							html += '				<a href="javascript:;" onclick="goAnalysisWeibo(1, \'' + n.retweetedStatus.url + '\', false)" class="link">分析</a>';
							html += '			</div>';
							html += '		</div>';
							html += '	</div>';
							html += '</div>';
						...
						html += '				<a href="javascript:;" onclick="goAnalysisWeibo(1, \'' + n.url + '\', false)" class="link">分析</a>';
						html += '			</div>';
						html += '		</div>';
						html += '	</li>';
						$('#navDiv').append(html);
					});
					$("#checkMyBoWen").css("padding-right",0);
				}
			}
		});
	}
		...
		success : function(result) {
			if (!$.isEmptyObject(result)) {
				if (result.status != null) {
					...
					shareWeiboContent = shareWeiboContent.replace(/<[^>]+>/g,"");//去除因表情展示的html标签
					...
					var anaTime = $("#anlysisTime").val();
					if(anaTime == null || anaTime == ''){
						anaTime = '2017-06-29 10:00:00';
					}
					anaTime = anaTime.split(":")[0]+":"+anaTime.split(":")[1];
					anaTime = anaTime.split("-")[1]+"-"+anaTime.split("-")[2];
					//总说明
					document.getElementById("analysis_time").innerHTML = anaTime;
...
function weiboTaskStatus() {
	$.ajax({
		...
		type : 'POST',
		data : {
			'fenxiWeibo.weiboId' : $('#weiboId').val(),
			'taskTicket' : $('#taskTicket').val(),
			'userId':$('#userId').val(),
			'taskCreateTime':$('#taskCreateTime').val(),
			'markType':$('#markType').val()
		...
		success : function(result) {
			if (!$.isEmptyObject(result)) {
				if (result.analysisStatus == 0) {
					...
					if (result.analysisStatus == 1||result.analysisStatus == 2) {
						console.log('status waiting...');
					} else if (result.analysisStatus == 3 ) {
						if (result.markType ==1 && result.schedulePercent >=1){
							$('#currentTaskTier').val(result.finishedTier);
							$('#sharePop').css("display","block");
							reloadWeiboTask();
							var finishPercent = result.schedulePercent.toFixed(0);
							$('.progressLine').css('width', finishPercent + '%');
							$('.progressLine').text(finishPercent + '%');
						}
						if (result.markType ==0 && result.finishedTier > $('#currentTaskTier').val()){
							$('#currentTaskTier').val(result.finishedTier);
							$('#sharePop').css("display","block");
							reloadWeiboTask();
							var finishPercent = (result.finishedTier / result.totalTier * 100).toFixed(0);
							$('.progressLine').css('width', finishPercent + '%');
							$('.progressLine').text(finishPercent + '%');
						}

					} else if (result.analysisStatus == 4) {
						console.log('waiting confirm analysis!');
					}
					if (result.schedulePercent < 1 ){
						$('.loadingMask .waiting').html('<div class="load-spot"><p class="text">Loading</p><div class="k-line k-line11-1"></div><div class="k-line k-line11-2"></div><div class="k-line k-line11-3"></div><div class="k-line k-line11-4"></div><div class="k-line k-line11-5"></div></div>');
				...
				// if (result.analysisStatus == 0 && result.weiboPayment == 1) {
				// 	$('.loadingMask .waiting').text('微博分析失败，请确认微博链接是否正确！');
				// } else {
				// 	if (result.analysisStatus != 1 && result.analysisStatus != 5 ) { // 待分析
				// 		console.log('status waiting...');
				// 	} else if (result.analysisStatus == 5 ) { //分析完成
				// 		reloadWeiboTask();
				// 	} else if (result.analysisStatus == 1 && result.weiboPayment == 0 ) { // 等待确认分析
				// 		console.log('waiting confirm analysis!');
				// 	}
				// 	if (result.analysisStatus != 5 && result.analysisStatus != 1) { // 未分析完成，则继续请求更新
				// 		$('.loadingMask .waiting').html('<div class="load-spot"><p class="text">Loading</p><div class="k-line k-line11-1"></div><div class="k-line k-line11-2"></div><div class="k-line k-line11-3"></div><div class="k-line k-line11-4"></div><div class="k-line k-line11-5"></div></div>');
				// 		setTimeout('weiboTaskStatus()', 5000);
				// 	} else {
				// 		$('.loadingData').text('微博分析完成');
...
//			_a = (parseFloat($('#cost').val()) - parseFloat($('#no_first_v_normal').val())).toFixed(2);
		html += '			<font class="f-d"></font>共为您节省<font class="f-r"></font>元';
		html += '		</span>';
		html += '	</li>';
		if ($('#firstBuy').val() == 'false') {
			html += '	<li>';
			html += '		<span><i class="yellow">多买惠</i>本月非首次购买，享' + ($('#discount_nofirstbuy').val() * 10) + '折</span>';
			html += '		<span class="float_r">立省<font class="f-r"></font>元</span>';
			html += '	</li>';
		}
		if ($('#firstBuy').val() == 'true') {
			html += '	<li><span class="f-g">还可享受的优惠</span></li>';
			html += '	<li>';
			html += '		<span class="f-g"><i>多买惠</i>本月首次购买，下次享' + ($('#discount_nofirstbuy').val() * 10) + '折</span>';
...
function checkSWAPaymentV2() {
	weiboTaskResultStatus(); // 获取微博内容
	if ($('#needPay').val() == 'true' && parseInt($('#repostsCount').val()) > 0) {
		var params = {packageCount : $('#repostsCount').val()};
		openBuyCommon(8, params);
		$('#myModalSWA').on('hide.bs.modal', function () {
			$('.loadingMask .waiting').html('<a class="closeBtn" href="javascript:;" onclick="javascript:location.href = \'' + actionBase + '/i/singleWeiboAnalysis/index.shtml\';">×</a>您还未支付分析费用<br />可前往分析记录中完成支付哦~^0^~');
		});
	} else {
		if ($('#taskTicket').val()) {
	...
	/*if ($('#needPay').val() == 'true' && parseInt($('#repostsCount').val()) > 0) {
		...
		openBuyCommon(8, params);
		if ($('#firstBuy').val() == 'true'){
			if(parseInt($('#repostsCount').val()) <=4000000){
				FirstBuyDiscountPackage_V2();
				$('#myModalSWA4F').modal('show');
				$('#myModalSWA4F').on('hide.bs.modal', function () {
					$('.loadingMask .waiting').html('<a class="closeBtn" href="javascript:;" onclick="javascript:location.href = \'' + actionBase + '/weibo/toAnalysis\';">×</a>您还未支付分析费用<br />可前往分析记录中完成支付哦~^0^~');
				});
			}else{
				unfirstBuyPop();
				$('#unfirst_buy_pop').modal('show');
				$('#unfirst_buy_pop').on('hide.bs.modal', function () {
					$('.loadingMask .waiting').html('<a class="closeBtn" href="javascript:;" onclick="javascript:location.href = \'' + actionBase + '/weibo/toAnalysis\';">×</a>您还未支付分析费用<br />可前往分析记录中完成支付哦~^0^~');
				});
			}
		}else{
			$('#myModalSWA4M').modal('show');
			$('#myModalSWA4M').on('hide.bs.modal', function () {
				$('.loadingMask .waiting').html('<a class="closeBtn" href="javascript:;" onclick="javascript:location.href = \'' + actionBase + '/weibo/toAnalysis\';">×</a>您还未支付分析费用<br />可前往分析记录中完成支付哦~^0^~');
			});
		}
	} else {
...
function checkSWAOrderFee() {
	$('#weiboConfirmOrderBtn').hide();
	// 获取待支付金额
	$.ajax({
		url : actionBase + '/view/pay/getOrderFee.action',
		type : 'POST',
		data : $('#swaBuyForm').serialize(),
		success : function(data) {
			if (!$.isEmptyObject(data)) {
				var fee = data.totalFee;
				var _a = (fee * parseFloat($('#discount_nofirstbuy').val())).toFixed(2);
				$('font.f-d').text('原价' + fee + '元');
				if ($('#firstBuy').val() == 'false') {
					$('font.f-t').text(_a);
					$('font.f-r').text((fee - _a).toFixed(2));
				} else {
					$('font.f-t').text(fee);
					$('font.f-r').text(0);
				}
				$('#weiboConfirmOrderBtn').show();
			}
		}
	});
}