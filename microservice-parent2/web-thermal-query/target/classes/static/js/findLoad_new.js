...
function setStartAndEndTime() {
    ...
    if((enddate-date)/(1000*60*60*24)>30){
        $("#combline_flg_1").hide();
    }
}
...
function showSubForm() {
    ...
    var startDate=new Date($("#showStarttime").val().replace(/-/g, "/"));
    if(start_stamp > end_stamp ){
        if(document.getElementById("dropdown_time")){
            document.getElementById("dropdown_time").style.display = "none";
		}
        showMsgInfo(0,"开始时间不能大于结束时间",0);
        return ;
    }
    /*var errorStartTime = 1483200000000;//2017-01-01
    var errorEndTime = 1494345599000;//2017-05-09
    if((start_stamp >= errorStartTime && start_stamp <= errorEndTime)
    		|| (end_stamp >= errorStartTime && end_stamp <= errorEndTime)){
    	showMsgInfo(0, '亲爱的微友们，微舆情平台正在维护2017年1月1日至5月9日这一时间段内的数据，若您下载该时间段内的数据，发现有数据缺失，经后台核实后，微舆情平台会通过微积分返还的方式进行补偿，若您对此做法不能接受，请谨慎下载数据，给您带来的不便，敬请谅解！', 0);
    	$(".submitBtn").one("click",function(){
    		
        });
    }*/
    /*if(end_stamp-start_stamp > 1000*60*60*24*getDays(startDate)){
        if(document.getElementById("dropdown_time")){
            document.getElementById("dropdown_time").style.display = "none";
        }
        showMsgInfo(0,"对不起，当前系统仅支持查询时间跨度为一个月的信息",0);
        return ;
	}*/
