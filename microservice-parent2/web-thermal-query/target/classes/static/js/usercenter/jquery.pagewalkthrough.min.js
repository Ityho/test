(function($, window, document, undefined) {
	...
	var methods = {
		...
		close: function() {
			...
			location.href = "account.shtml?guide=0";
		},
	...
	function buildWalkthrough() {
		...
		if (step.popup.type !== "modal" && step.popup.type !== "nohighlight") {
			...
			$("<div>").addClass("overlay-hole").height(Math.min(maxHeight, targetElement.outerHeight())).width(targetElement.outerWidth()).css({
				padding: "20px",
				position: "absolute",
				top: targetElement.offset().top -2,
				left: targetElement.offset().left,
				"z-index": 999998,
				"box-shadow": "0 0 1px 10000px rgba(0, 0, 0, 0.7)"
			}).append($("<div>").css({
				position: "absolute",
				top: 0,
				bottom: 0,
	...
	$(document).on("click", "#jpwNext", function() {
		_activeWalkthrough.num++;
		if(_activeWalkthrough.num==5){
			$(".demo1-1").hide();
			$(".demo1-2").show();
		}
