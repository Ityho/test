...
public class RedisUtils {
    ...
    static void setRedisTemplate(RedisTemplate rTemplate ){
        redisTemplate = (StringRedisTemplate) rTemplate;
    }
...
//    }
     ...
     *            值
     * @param second
     *            有效期，单位：秒（默认86400秒 / 24小时）0:删除key
     * @return
     */
    public static void setAttribute(String key, String value, int second) {
        if (StringUtils.isNotBlank(key) && StringUtils.isNotBlank(value)) {
    ...
    public static String getAttribute(String key) {
        ...
        try {
            rtnStr = (String) redisTemplate.opsForValue().get(key);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
     ...
     * @param value
     */
    public static void refreshAttribute(String key, String pattern, String value, Integer second) {
        if ((StringUtils.isNotBlank(key) || StringUtils.isNotBlank(pattern)) && StringUtils.isNotBlank(value)) {
            ...
            if (CollectionUtils.isNotEmpty(keys)) {
                for (String k : keys) {
                    ...
                    if (ttl == null || ttl < 0) {
                        ...
                        setAttribute(k, value, Integer.parseInt(ttl.toString()));
                    }
                }
            }else if(StringUtils.isNotBlank(key)){
                if(second != null && second > 0){
                    setAttribute(key, value, second);
                }else{
     ...
     *
     * @param userId
     * @return true:加锁成功 false:锁存在
     */
    public static boolean lockUserOperate(Integer userId) {
        return lockUserOperate(userId, 2);
    }

    public static boolean lockUserOperate(String key) {
        return lockUserOperate(key, 2);
    }

    public static boolean lockUserOperate(Integer userId, int lockSecond) {
        return lockUserOperate(RedisUtils.generateJedisKey(SystemConstants.JEDIS_KEYS_USER_OPERATE_LOCK + userId));
    }

    public static boolean lockUserOperate(String lockKey, int lockSecond) {
        if (lockKey != null) {
            String lock = RedisUtils.getAttribute(lockKey);
            if(lock != null && "1".equals(lock)){
                return false;
            }else{
                RedisUtils.setAttribute(lockKey, "1", lockSecond);
                return true;
            }
        }
        return false;
     ...
     * @return
     */
    public static void unLockUserOperate(Integer userId) {
        if (userId != null) {
            removeAttribute(RedisUtils.generateJedisKey(SystemConstants.JEDIS_KEYS_USER_OPERATE_LOCK + userId));
        }
    }
}
